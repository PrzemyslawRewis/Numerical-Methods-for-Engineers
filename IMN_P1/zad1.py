...
if __name__ == '__main__':
    ...
    for delt in kroki_czasowe:
        ...
        numeryczne = open(f'./rozwiazanie_numeryczne{delt}.csv', 'w+')  # w+ tryb odczyt zapis
        analityczne = open('./rozwiazanie_analityczne.csv', 'w+')
        ...
        for i in np.arange(1, n):
            y_arg[i] = y_arg[i - 1] + delt * lamb * y_arg[i - 1]
            krok[i] = i * delt
